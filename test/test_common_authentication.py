...
class TestCommonAuthApi(TestCase):    
    ...
    def test_add_auth_details_to_configuration_config_values(self):
        ...
        api_auth_util = ApiAuthUtil(configuration=self.test_configuration)
        basicauthutil.add_basic_auth_details_to_configuration(self.test_configuration, username="jbond", password=basicauthutil.fetch_password_from_user_settings())
    ...
    def test_basic_auth_password_exception(self, mock_user_settings_get_user_password, mock_fetch_password_from_user_settings):
        ...
        with self.assertRaises(AsdkApiException) as context:
            ...
            self.assertTrue("Incomplete Basic Auth Details provided: Missing username and/or password. Please check AladdinSDK documentation for configuration details." in context.exception)
...
class TestCommonAuthApiInLocalWithKeyring(TestCase):    
    ...
    def test_add_auth_details_to_configuration_keyring_password(self, mock_keyring_util):
        from aladdinsdk.common.authentication.basicauth.basicauthutil import add_basic_auth_details_to_configuration, fetch_password_from_user_settings
        from aladdinsdk.common.authentication.api import ApiAuthUtil
...
class TestCommonAuthApiMissingConfigurationScenario(TestCase):    
    ...
    def test_add_auth_details_to_configuration_failure_missing_password(self):
        ...
        with self.assertRaises(Exception) as context:
            ...
            self.assertTrue('Insufficient API initialization information for Basic Auth. Password not passed/configured for SDK.' in context.exception)
...
class TestFetchOauthTokenFromParams(TestCase):    
    ...
    def test_oauth_request_headers_built_successfully_from_params(self, oauth_token, secrets):
        from aladdinsdk.common.authentication.api import ApiAuthUtil
        api_auth_util = ApiAuthUtil(auth_type="OAuth", configuration=self.test_configuration, username='test', client_id='id', client_secret='secret', refresh_token='token')
    ...
    @mock.patch('aladdinsdk.common.authentication.api.oauth_token_cred_client.get_access_token_and_ttl_from_oauth_server', return_value=["ACCESS_TOKEN456", 3600])
    @mock.patch('aladdinsdk.common.secrets.fsutil.read_secret_from_file', return_value=None)
    ...
    @mock.patch('aladdinsdk.common.authentication.api.oauth_token_cred_client.get_access_token_and_ttl_from_oauth_server', return_value=(None, None))
    @mock.patch('aladdinsdk.common.authentication.api.okta_sidecar_client.get_access_token_ttl_from_okta_sidecar', return_value=(None, None))
    ...
    @mock.patch('aladdinsdk.common.authentication.api.oauth_token_cred_client.get_access_token_and_ttl_from_oauth_server', return_value=(None, None))
    @mock.patch('aladdinsdk.common.authentication.api.okta_sidecar_client.get_access_token_ttl_from_okta_sidecar', return_value=("ACCESS_GRANTED", datetime.datetime(2021, 1, 1, 0, 0, 0, 0)))
    ...
    @mock.patch('aladdinsdk.common.authentication.api.oauth_token_cred_client.get_access_token_and_ttl_from_oauth_server', return_value=(None, None))
    @mock.patch('aladdinsdk.common.authentication.api.okta_sidecar_client.get_access_token_ttl_from_okta_sidecar', return_value=("ACCESS_GRANTED", None))
