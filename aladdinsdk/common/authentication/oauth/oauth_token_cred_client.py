...
def get_access_token_and_ttl_from_oauth_server(client_id=None, client_secret=None, refresh_token=None, scopes=None, auth_flow_type=None, auth_server_proxy=None, auth_server_url=None):
    """ 
    ...
    try:
        if (client_id and client_secret) is None and auth_flow_type == user_settings.CONF_API_AUTH_FLOW_TYPE_CLIENT_CREDENTIALS:
            raise AsdkOAuthException(f"Missing Oauth value(s): client-id/client-secret for auth flow type = client credentials. {SDK_HELP_MESSAGE_SUFFIX}")
        if (client_id and client_secret and refresh_token) is None and auth_flow_type == user_settings.CONF_API_AUTH_FLOW_TYPE_REFRESH_TOKEN:
            raise AsdkOAuthException(f"Missing Oauth value(s): client-id/client-secret/refresh-token for auth flow type = refresh token. {SDK_HELP_MESSAGE_SUFFIX}")
        if (refresh_token == "<no value>") and auth_flow_type == user_settings.CONF_API_AUTH_FLOW_TYPE_REFRESH_TOKEN:
            raise AsdkOAuthException(f"Missing refresh token. Please login to studio application to create refresh token. {SDK_HELP_MESSAGE_SUFFIX}")
        
        access_token_ttl_tuple = _retrieve_token_details(client_id, client_secret, refresh_token, auth_flow_type, scopes, auth_server_proxy, auth_server_url)
