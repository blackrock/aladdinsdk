...
class ApiAuthUtil():
    ...
    def __init__(self,
        ...
        **kwargs):
        ...
        Args:
            ...
            api_key (_type_, optional): API Key. Defaults to value set as "ASDK_API__TOKEN" environment variable, or "api.token" in settings yaml, None if not configured.
            configuration (_type_, optional): API confiuration. Defaults to value None if not configured.
            auth_type (_type_, optional): API Authentication Type. Must be in ["Basic Auth", "OAuth"]. Defaults to value set as "ASDK_API__AUTH_TYPE" or "api.auth_type" in settings yaml.
            auth_flow_type (_type_, optional): : API Authentication Type. Must be in ["refresh_token", "client_credentials"]. Defaults to value set as "ASDK_API__AUTH_FLOW_TYPE" or "api.auth_flow_type" in settings yaml.
            username (_type_, optional): Username. Defaults to value set as "ASDK_USER_CREDENTIALS__USERNAME" environment variable, or "user_credentials.username" in settings yaml, None if not configured.
            password (_type_, optional): Password. Defaults to value set as "ASDK_USER_CREDENTIALS__PASSWORD" environment variable, or "user_credentials.password" in settings yaml, None if not configured.
            client_id (_type_, optional): Client Id for Oauth. Defaults to value set as "ASDK_OAUTH__CLIENT_ID" environment variable, or "oauth.client_id" in settings yaml, None if not configured.
            client_secret (_type_, optional): Client Secret for Oauth. Defaults to value set as "ASDK_OAUTH__CLIENT_SECRET" environment variable, or "oauth.client_secret" in settings yaml, None if not configured.
            refresh_token (_type_, optional): Refresh token to use for oauth token uri. Defaults to value set as "ASDK_OAUTH__REFRESH_TOKEN" environment variable, or "oauth.refresh_token" in settings yaml, None if not configured.
            ...
            auth_server_proxy (_type_, optional): API Auth Server Proxy for Oauth. Defaults to value set as "ASDK_OAUTH__AUTH_SERVER_PROXY" environment variable, or "oauth.auth_server_proxy" in settings yaml, None if not configured.
            auth_server_url (_type_, optional): API Auth Server URL for Oauth. Defaults to value set as "ASDK_OAUTH__AUTH_SERVER_URL" environment variable, or "oauth.auth_server_url" in settings yaml, None if not configured.
            password_filepth (_type_, optional): Basic Auth Password Filepath. Defaults to value set as "ASDK_USER_CREDENTIALS__PASSWORD_FILEPATH" environment variable, or "user_credentials.password_filepath" settings yaml, None is not configured.
            encrypted_password_filepath (_type_, optional): Basic Auth Password Filepath. Defaults to value set as "ASDK_USER_CREDENTIALS__ENCRYPTED_PASSWORD_FILEPATH" environment variable, or "user_credentials.encrypted_password_filepath" settings yaml, None is not configured.
            encryption_filepath (_type_, optional): Basic Auth Encryption Filepath. Defaults to value set as "ASDK_USER_CREDENTIALS__ENCRYPTION_FILEPATH" environment variable, or "user_credentials.encryption_filepath" settings yaml, None is not configured.
        """
        ...
        self.user_provided_api_access_token = self._fetch_oauth_param(self.api_access_token, 'access_token') if self.auth_type == user_settings.CONF_API_AUTH_TYPE_OAUTH else None
    ...
    def add_auth_details_to_header_and_config(self, scopes=None):
        ...
        In case of basic auth: adding api key header to configuration + adding basic auth details to configuration + return basic auth headers for api call
        In case of oauth: adding api key header to configuration + add oauth api access token to auth header + return oauth headers for api call
        ...
        if self.auth_type == user_settings.CONF_API_AUTH_TYPE_BASIC_AUTH:
            ...
            if self.password is None:
                ...
                self.password = basicauthutil.fetch_password_from_user_settings(self.password_filepath, self.encrypted_password_filepath, self.encryption_filepath)
            basicauthutil.add_basic_auth_details_to_configuration(self.configuration, username=self.username, password=self.password)        
    ...
    def _request_oauth_access_token_tuple(self, scopes):
        ...
        access_token_ttl_tuple_from_auth_server = oauth_token_cred_client.get_access_token_and_ttl_from_oauth_server(client_id, client_secret, refresh_token, scopes, self.auth_flow_type, self.auth_server_proxy, self.auth_server_url)
    ...
    def _fetch_oauth_param(self, param_value, param_name):
        ...
        This method assumes the DYNACONF user settings get methods for fetching a config key value follows the format: get_api_oauth_OAUTH_PARAM_NAME_HERE()
        This method also assumes the DYNACONF user settings get methods for fetching a config key value filepath follows the format: get_api_oauth_OAUTH_PARAM_NAME_HERE_filepath()
