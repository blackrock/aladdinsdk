...
class APIExceptionHandler(AbstractAsdkExceptionHandler): # e001
    ...
    def _handle_unauthorized_exception(self):
        if user_settings.get_run_mode() == user_settings.CONF_RUN_MODE_LOCAL and user_settings.get_user_password() == None and user_settings.get_api_auth_type() == user_settings.CONF_API_AUTH_TYPE_BASIC_AUTH:
            _logger.warning("UnauthorizedException occurred while running locally. Could be invalid credential stored in OS Credential Manager. Deleting entry using 'keyring'. Re-enter password for next attempt.")
            keyringutil.delete_user_password()
        else:
            _logger.warning(f"UnauthorizedException occurred while attempting to run AladdinSDK API client instance. AladdinSDK-APIException unable to handle error. {SDK_HELP_MESSAGE_SUFFIX} on settings up proper authentication for AladdinSDK")
