...
def exception_type_matches(supported_exception_list_entry, original_exception):
    ...
    Handlers include a list of exceptions that can be handled. Supported exceptions can be listed as one of:
        ...
        - string - containing a wildcard to cover all modules under a given path. e.g.: "aladdinsdk.api.codegen.*UnauthorizedException": covers all UnauthorizedException classes under any API's openapi generated code under aladdinsdk.api.codegen
    ...
    """
    if type(supported_exception_list_entry) == type:
        return type(original_exception) == supported_exception_list_entry
