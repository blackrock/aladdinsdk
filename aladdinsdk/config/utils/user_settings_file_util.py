...
from aladdinsdk.config.user_settings import CONF_API_AUTH_TYPE_OAUTH, CONF_API_AUTH_TYPE_BASIC_AUTH, CONF_RUN_MODE_LOCAL, CONF_RUN_MODE_ALADDIN_COMPUTE, ALADDIN_COMPUTE_CLIENT_REFRESH_TOKEN_PATH, ALADDIN_COMPUTE_CLIENT_DETAILS_PATH, ALADDIN_COMPUTE_SECRETS_KEYENC_PATH
...
def print_user_config_file_template():
    ...
    This methods looks for NB_USER environment variable to deduce run mode, and presence of secret files at default locations to add as much detail as possible in the configuration.
    """      
    _asdk_banner = """
...
def _prompt_user_for_api_key():
    return input("As part of the configuration for making API calls, and API Key is required. Please enter a valid key registered from your profile within Studio: ")
...
def print_current_user_config():
    ...
    # Hide user password
    if 'USER_CREDENTIALS' in conf_copy.keys() and 'PASSWORD' in conf_copy['USER_CREDENTIALS'].keys() and conf_copy['USER_CREDENTIALS']['PASSWORD'] is not None:
