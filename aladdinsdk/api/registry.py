...
class AladdinAPICodegenDetails:
    ...
    def __init__(self, api_name, api_version, api_module_path, host_url_path, swagger_file_path):
        ...
        # To list callable endpoint methods, list out all methods in the class except private (starts with '_') or helpers (ends with '_with_http_info')
...
def get_api_details(api_name, api_version=None) -> AladdinAPICodegenDetails:
    ...
    if api_version is not None:
        ...
        api_versions.sort() # # if versions are in semantic version, split each version string to compare it as a list of integers. Ref. https://stackoverflow.com/questions/2574080/sorting-a-list-of-dot-separated-numbers-like-software-versions use this --> api_versions.sort(key=lambda s: list(map(int, s.split('.')))) 
...
for ae in _sdk_supported_apis:
    try:
        ...
        _api_codegen_detail = AladdinAPICodegenDetails(ae['api_name'], ae['api_version'], ae['api_module_path'], ae['host_url_path'], ae['swagger_file_path'])
...
if _reg_failed_counter > 0:
    _logger.debug("Internal SDK setup warning. Unable to fully setup API registry. Potentially malformed codegen allow list configuration. Review CODEGEN_API_SETTINGS.ALLOW_LIST.")
