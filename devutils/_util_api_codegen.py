...
Sample invocation:
    - Onboarding new API  / Re-creating existing API:
        python _util_api_codegen.py onboard UserGroup -agr /path-to/aladdin-graph -s platform/platform/access/user_group/v1/user_group_api.proto
...
def _onboard_api():
    ...
    with tempfile.TemporaryDirectory() as tdir:
        ...
        python_codegen_command = ["java", "-jar", local_api_generator_path, "generate",
                                  "-i", __agraph_gen_swagger_file_path, "-o", tdir,
                                  "-g", "python-nextgen", "--package-name", __target_api_module_import,
                                  "--api-name-suffix", api_name, "--strict-spec", "false"]
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(prog="ASDK Codegen Util",
                                     description="Utility to help SDK developers onboard (or delete) APIs from aladdin-graph.")
    parser.add_argument('mode',
                        help=f"Run mode for this program. Currently supports [{_RUN_MODE_ONBOARD}, {_RUN_MODE_DELETE}].")
    parser.add_argument('-oas', '--openapi-spec',
                        help="(onboard mode - spec) Openapi Spec file path - to be used instead of agraph repo's proto file, "
                        "for onboarding using a pre-existing swagger specification.")
    parser.add_argument('-agr', '--aladdin-graph-repo',
                        help="(onboard mode - proto) AGraph Repo - Absolute path to aladdin-graph-repo checkout out locally.")
    parser.add_argument('-s', '--source-api-proto-path',
                        help="(onboard mode - spec/proto) Source file - Relative path from aladdin-graph/defs to API proto file "
                        "which needs to be onboarded.")
    parser.add_argument('-dan', '--delete-api-name',
                        help="(delete mode) Name of API to delete.")
    parser.add_argument('-dav', '--delete-api-version',
                        help="(delete mode) API version to delete.")
    ...
    if __codegen_util_run_mode == _RUN_MODE_ONBOARD:  # If run mode, derive additional constants needed for program execution
        ...
        __agraph_gen_swagger_file_path = __path_to_openapi_spec_file if __path_to_openapi_spec_file is not None else \
            f"gen/swagger/{__path_to_target_api_module}_api.swagger.json"
